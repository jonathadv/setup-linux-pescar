...
function configure_firefox_profile(){
    ...
    if [[ ! -d "${firefox_profile_basedir}" ]]; then
        logger "directory ${firefox_profile_basedir} does not exist. Starting and stopping firefox to create it!"
        
        sudo -u pescar firefox 2> /dev/null &
        sleep 5

        while ! pidof firefox; do
            logger "Waiting firefox"
            sleep 1
        done
    ...
    if ls -d ${firefox_profile_basedir}/*.default-* 2> /dev/null; then
        ...
        backup "${firefox_profile_basedir}/${current_firefox_profile_name}"
        rm -rf "${firefox_profile_basedir}/${current_firefox_profile_name}"

    else
        logger "Unable to find any profile folder under \"${firefox_profile_basedir}\". No need to backup."
    fi
